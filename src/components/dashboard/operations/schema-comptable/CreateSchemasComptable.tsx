...
import type { CreateSchemaComptable, CreateEcritureComptable } from "@/types/schemaComptable.types";
import { EcritureComptableValidator, SchemaComptableValidator } from "@/types/schemaComptable.types";

interface CreateSchemaComptableProps {
  typesOperation: TypeOperation[];
  modesReglement: ModeReglement[];
  typesCompte: CompteType[];
  typesMontant: TypeMontant[];
  onSubmit: (schema: CreateSchemaComptable) => void;
  onCancel: () => void;
}

export default function CreateSchemaComptable({ 
  typesOperation, 
  modesReglement, 
  typesCompte,
  typesMontant,
  ...
  const [ecritures, setEcritures] = useState<CreateEcritureComptable[]>([]);
  const [errors, setErrors] = useState<string[]>([]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const canShowEcritureForm = selectedTypeOp > 0 && selectedModeReglement > 0;
  const canSubmit = canShowEcritureForm && ecritures.length > 0;

  const isFormValid = () => {
    if (!canSubmit) return false;
    return ecritures.every(ecriture => 
      EcritureComptableValidator.validate(ecriture).length === 0
    );
  };
  ...
  const onTypeOperationChange = (typeOpId: number) => {
    ...
    if (typeOpId === 0) {
      setEcritures([]);
    } else if (selectedModeReglement > 0) {
      initializeFirstEcriture();
    }
  };
  ...
  const onModeReglementChange = (modeReglementId: number) => {
    ...
    if (modeReglementId > 0 && selectedTypeOp > 0) {
      initializeFirstEcriture();
    }
  };

  // Initialise la première écriture
  const initializeFirstEcriture = () => {
    setEcritures([createNewEcriture()]);
  };

  // Créé une nouvelle écriture vide
  const createNewEcriture = (): CreateEcritureComptable => ({
    sens: 'DEBIT',
    id_typeCompte: 0,
    type_detenteur: true,
    id_typeMontant: 0
  ...
  const updateEcriture = (index: number, field: keyof CreateEcritureComptable, value: any) => {
    setEcritures(ecritures.map((ecriture, i) => 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const schema: CreateSchemaComptable = {
      id_reglement: selectedModeReglement,
      id_tyOp: selectedTypeOp,
      ecritures: ecritures
    };

    const validationErrors = SchemaComptableValidator.validate(schema);
  ...
  return (
    <div className="create-schema-container bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-amber-200/30 dark:border-amber-700/30">
      <div className="form-header mb-6">
        <h3 className="text-xl font-bold text-amber-900 dark:text-amber-100">Nouveau Schéma Comptable</h3>
      </div>
        ...
        {canShowEcritureForm && (
          <div className="ecritures-section mb-6">
            <div className="section-header flex justify-between items-center mb-4">
              <h4 className="text-lg font-semibold text-amber-900 dark:text-amber-100">Écritures Comptables</h4>
              <button 
                type="button" 
              ...
              <table className="ecritures-table w-full">
                <thead>
                  <tr className="border-b border-amber-200/30 dark:border-amber-700/30">
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100 w-12">#</th>
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100">Sens *</th>
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100">Type de Compte *</th>
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100">Type de Montant *</th>
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100">Détenteur *</th>
                    <th className="p-3 text-left font-medium text-amber-900 dark:text-amber-100 w-20">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {ecritures.map((ecriture, index) => (
                    <tr key={index} className="border-b border-amber-100/50 dark:border-amber-800/30">
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <select 
                          value={ecriture.sens}
                          onChange={(e) => updateEcriture(index, 'sens', e.target.value)}
                      ...
                      <td className="p-3">
                        <select 
                          ...
                          onChange={(e) => updateEcriture(index, 'id_typeCompte', Number(e.target.value))}
                          className="form-input w-full px-2 py-1 bg-amber-50/50 dark:bg-amber-900/20 border border-amber-200/30 dark:border-amber-700/30 rounded-lg focus:ring-1 focus:ring-amber-500 focus:border-transparent"
                          required
                        >
                      ...
                      <td className="p-3">
                        <select 
                          ...
                          onChange={(e) => updateEcriture(index, 'id_typeMontant', Number(e.target.value))}
                          className="form-input w-full px-2 py-1 bg-amber-50/50 dark:bg-amber-900/20 border border-amber-200/30 dark:border-amber-700/30 rounded-lg focus:ring-1 focus:ring-amber-500 focus:border-transparent"
                          required
                        >
                          <option value={0}>Sélectionnez...</option>
                      ...
                      <td className="p-3">
                        <select 
                          ...
                          onChange={(e) => updateEcriture(index, 'type_detenteur', e.target.value === 'true')}
                          className="form-input w-full px-2 py-1 bg-amber-50/50 dark:bg-amber-900/20 border border-amber-200/30 dark:border-amber-700/30 rounded-lg focus:ring-1 focus:ring-amber-500 focus:border-transparent"
                          required
                        >
