...
export default function PermissionForm({
  ...
  // Mettre à jour le formulaire si la permission change
  useEffect(() => {
    if (permission) {
      setFormData({
        name: permission.name,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      validationErrors.push("Le nom de la permission est requis");
    }
    
    if (formData.name.length > 50) {
      validationErrors.push("Le nom de la permission ne doit pas dépasser 50 caractères");
    }
    ...
    if (validationErrors.length === 0) {
      ...
      if (isEdit && permission) {
        // Mode modification
        onSubmit({ ...formData, id: permission.id });
      } else {
        // Mode ajout
        onSubmit(formData);
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="permission-form-pro">
